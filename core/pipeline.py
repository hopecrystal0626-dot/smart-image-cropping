...
def process_image(img_path, use_depth=True, save_vis=True, output_dir=None, progress_callback=None):
    """
    ...
    Args:
        ...
        progress_callback: 可选回调函数，接受 (progress_percent, status_message)
    Returns:
        dict: {
            ...
            'top10': list of dict,
            'top10_crops': list of np.ndarray,
            'grid_image': np.ndarray (可选),
            'all_records': list,
            'depth_vis': np.ndarray (RGB, 彩色深度图) or None,
            'seg_vis': np.ndarray (RGB, 彩色分割图) or None
        }
    """
    if progress_callback:
        progress_callback(5, "初始化...")
    ...
    if depth_map is not None:
        ...
        depth_vis = cv2.cvtColor(depth_color, cv2.COLOR_BGR2RGB)   # 转为 RGB

    # 9. 生成分割图彩色可视化
    seg_vis = None
    if seg_map is not None:
        # seg_map 可能是 Tensor，转为 numpy
        if hasattr(seg_map, 'cpu'):
            seg_map_np = seg_map.cpu().numpy()
        else:
            seg_map_np = seg_map
        np.random.seed(42)
        colors = np.random.randint(0, 255, (seg_map_np.max() + 1, 3), dtype=np.uint8)
        seg_color = colors[seg_map_np]   # shape (H, W, 3) RGB
        seg_vis = seg_color

    if progress_callback:
        progress_callback(95, "准备完成")
