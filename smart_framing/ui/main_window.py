# -*- coding: utf-8 -*-
"""
MainWindow 主窗口 - 适配队友 process_image 接口，无 torch 依赖
"""
import os
import sys
...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QGroupBox, QSizePolicy, QComboBox, QScrollArea, QApplication
)
...
from image_processing import apply_adjustments, apply_aspect_ratio


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AIPhotoCrop · 智能取景器")
        self.setGeometry(100, 50, 1500, 950)
        self.setStyleSheet(self.get_style())

        # ---- 状态 ----
        self.img_path = None
        self.original_img = None          # BGR 原图
        self.cropped_img = None           # BGR 当前显示裁剪图
        self.candidates = []              # 每个元素: {x1,y1,x2,y2, final_score, crop_image}
        self.current_idx = 0
        self.top_n = 10
    ...
    def init_ui(self):
        ...
        self.depth_label = QLabel("队友算法提供")
        ...
        self.mask_group = QGroupBox("语义分割")
        mask_layout = QVBoxLayout()
        self.mask_label = QLabel("队友算法提供")
    ...
    def get_style(self):
        return """
            QMainWindow { background: #DCE8F5; }
            QLabel { color: #2C3E50; }
            QSplitter::handle { background: #B8CEE4; }
            QToolTip { background: #F2F6FA; color: #2C3E50; border: 1px solid #B8CEE4; }
            QGroupBox {
                border: 1px solid #B8CEE4;
                border-radius: 6px;
                margin-top: 12px;
    ...
    def get_display_image(self):
        ...
        if self.current_mode == self.MODE_PREVIEW:
            ...
            right = self.cropped_img.copy() if all(v == 0 for v in self.params.values()) else apply_adjustments(self.cropped_img, **self.params)
    ...
    # ---------- 深度图/分割图辅助显示 ----------
    def _show_cv_in_label(self, img_bgr, label, placeholder=""):
        """将 BGR ndarray 缩放后渲染到 QLabel；img_bgr 为 None 时显示占位文字。"""
        if img_bgr is None:
        ...
        qt_img = QImage(resized.data, new_w, new_h, bytes_per_line, QImage.Format_RGB888)
    ...
    def load_image(self):
        ...
        self.depth_label.setText("（加载中...）")
        self.mask_label.setPixmap(QPixmap())
        self.mask_label.setText("（加载中...）")
        ...
        # ---- 启动后台线程 ----
        self.load_thread = LoadWorker(self.img_path)
    ...
    def update_progress_smooth(self):
        if self.simulated_progress < 90:
            self.simulated_progress += 1
            self.workflow.progress_bar.setValue(self.simulated_progress)
            self.workflow.status_label.setText(f"处理中... {self.simulated_progress}%")
        else:
            self.progress_timer.stop()

    def on_load_progress(self, value, msg):
        if value > self.simulated_progress:
            self.simulated_progress = value
    ...
    # ★ 关键修复：签名增加 depth_bgr / saliency_bgr 两个参数
    def on_load_finished(self, candidates_list, img_bgr, grid_bgr, depth_bgr, saliency_bgr):
        ...
        if candidates_list and len(candidates_list) > 0:
            ...
            if 'crop_image' in best and best['crop_image'] is not None:
                self.cropped_img = best['crop_image']
            else:
        ...
        # ★ 深度图 & 分割图渲染到右侧面板
        self._show_cv_in_label(depth_bgr, self.depth_label, "（无深度图）")
        self._show_cv_in_label(saliency_bgr, self.mask_label, "（无分割图）")
    ...
    def confirm_crop(self):
        ...
        QMessageBox.information(self, "成功", "裁剪已应用，原候选框保持不变")
    ...
    def save_result(self):
        if self.cropped_img is None:
            ...
            return
        result_img = self.cropped_img.copy() if all(v == 0 for v in self.params.values()) else apply_adjustments(self.cropped_img, **self.params)
    ...
    def update_thumbs(self):
        self.thumb_list.clear()
        if self.original_img is None or not self.candidates:
            return
        ...
        for idx, cand in enumerate(self.candidates[:self.top_n]):
            if 'crop_image' in cand and cand['crop_image'] is not None:
    ...
    def on_thumb_clicked(self, item):
        ...
        if idx is not None and idx < len(self.candidates):
            ...
            if 'crop_image' in cand and cand['crop_image'] is not None:
                self.cropped_img = cand['crop_image']
            else:
                x1, y1, x2, y2 = int(cand['x1']), int(cand['y1']), int(cand['x2']), int(cand['y2'])
                self.cropped_img = self.original_img[y1:y2, x1:x2]
            self.reset_params(silent=True)
